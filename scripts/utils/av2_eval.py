...
# {<MetricBreakdownCategories.BACKGROUND: 'Background'>: [0], 
# <MetricBreakdownCategories.FOREGROUND: 'Foreground'>:
# [5, 8, 9, 13, 21, 22, 1, 10, 16, 17, 3, 4, 14, 15, 23, 28, 29, 30, 2, 6, 7, 11, 12, 18, 19, 20, 24, 25, 26, 27]}
...
def get_carterian_res(pc, sensor):
    ...
    if sensor == 'radar': # LRR30
        ...
        res = np.array([0.2, 1.5 * np.pi/180, 1.5 * np.pi/180])
    ...
    if sensor == 'lidar': # HDL-64E
        ...
        res = np.array([0.04, 0.4 * np.pi/180, 0.08 *np.pi/180])
    ...
    r = np.sqrt(x**2+y**2+z**2) # 256,
    theta = np.arcsin(z/r)
    phi = np.arctan2(y,x)

    ## compute xyz's gradient about r/theta/phi 
    grad_x = np.stack((np.cos(phi)*np.cos(theta), -r*np.sin(theta)*np.cos(phi), -r*np.cos(theta)*np.sin(phi)),axis=1)
    grad_y = np.stack((np.sin(phi)*np.cos(theta), -r*np.sin(phi)*np.sin(theta), r*np.cos(theta)*np.cos(phi)),axis=1)
    grad_z = np.stack((np.sin(theta), r*np.cos(theta), np.zeros(np.size(x,0))),axis=1) # 256，3

    # x_res = np.zeros(np.size(x,0))
    # y_res = np.zeros(np.size(x,0))
    # z_res = np.zeros(np.size(x,0))


    # for id in range(pc.shape[0]):
    #     x_res[id] = abs(grad_x[id,0]) * res[0] + abs(grad_x[id,1]) * res[1] + abs(grad_x[id,2])* res[2]
    #     y_res[id] = abs(grad_y[id,0]) * res[0] + abs(grad_y[id,1]) * res[1] + abs(grad_y[id,2])* res[2]
    #     z_res[id] = abs(grad_z[id,0]) * res[0] + abs(grad_z[id,1]) * res[1] + abs(grad_z[id,2])* res[2]



    # measure resolution for xyz (different positions have different resolution)
    x_res = np.sum(abs(grad_x) * res,axis=1)
    y_res = np.sum(abs(grad_y) * res,axis=1)
    z_res = np.sum(abs(grad_z) * res,axis=1)


    xyz_res = np.stack((x_res,y_res,z_res),axis=1)
...
def compute_metrics(
    ...
    Args:
        pred_flow: (N,3) Predicted flow vectors. 自运动流叠加本网络估计的流的结果
        pred_dynamic: (N,) Predicted dynamic labels. 动态前景mask
        gts: (N,3) Ground truth flow vectors. 来自数据集的场景流真值 自运动流叠加动态前景流
        category_indices: (N,) Integer class labels for each point. 来自 batch['flow_category_indices'][batch_id][valid_from_pc2res]
        is_dynamic: (N,) Ground truth dynamic labels. 由场景流真值检测的动态前景mask
        is_close: (N,) True for a point if it is within a 70m x 70m box around the AV. 是否在以自车为中心35m范围内
        is_valid: (N,) True for a point if its flow vector was successfully computed. 来自 batch['flow_is_valid'][batch_id][valid_from_pc2res]
    ...
    all_results: DefaultDict[str, List[Any]] = defaultdict(list)
    ...
    # metric_categories.items() 前景 or 背景
    ...
    ALL_EPE3D = compute_epe(results, [2, 3, 4, 5, 6, 7])

    all_mask = all_results["mask"][2] | all_results["mask"][3] | all_results["mask"][4] | all_results["mask"][5] | all_results["mask"][6] | all_results["mask"][7] 


    gts_sub = gts[all_mask]
    pred_sub = pred_flow[all_mask]
    for flow_metric_type in SceneFlowMetricType:
        all_results[flow_metric_type] = compute_scene_flow_metrics(pred_sub, gts_sub, flow_metric_type).mean()
    ...
    return {
        ...
        'ALL_EPE3D': ALL_EPE3D,
        'ALL_ACCURACY_RELAX': all_results["ACCURACY_RELAX"],
        'ALL_ACCURACY_STRICT' : all_results["ACCURACY_STRICT"],
        'ALL_ANGLE_ERROR' : all_results["ANGLE_ERROR"],
        'ALL_EPE' :all_results["EPE"]
    }
...
def compute_radar_metrics(
    ...
    gts = gts[is_valid].astype(np.float64)
    is_dynamic = is_dynamic[is_valid].astype(bool)
    pc = pc[is_valid].astype(np.float64)

    error = np.sqrt(np.sum((pred_flow - gts)**2, axis=1) + 1e-20)
    gtflow_len = np.sqrt(np.sum(gts*gts, axis=1) + 1e-20) 
    ## compute traditional metric for scene flow
    epe = np.mean(error)

    accs = np.sum(np.logical_or((error <= 0.05), (error/gtflow_len <=0.05)))/(np.size(pred_flow,0))
    accr = np.sum(np.logical_or((error <= 0.10), (error/gtflow_len <=0.10)))/(np.size(pred_flow,0))

    ## obtain x y z measure resolution for each point (radar lidar)
    xyz_res_r = get_carterian_res(pc, 'radar') 
    res_r = np.sqrt(np.sum(xyz_res_r,1)+1e-20)
    xyz_res_l = get_carterian_res(pc, 'lidar') 
    res_l = np.sqrt(np.sum(xyz_res_l,1)+1e-20)

    ## calcualte Resolution-Normalized Error
    re_error = error/(res_r/res_l)
    rne = np.mean(re_error)
    mov_rne = np.sum(re_error[is_dynamic==1])/(np.sum(is_dynamic==1)+1e-6)
    stat_rne = np.mean(re_error[is_dynamic==0])
    avg_rne = (mov_rne+stat_rne)/2

    ## calculate Strict/Relaxed Accuracy Score
    sas = np.sum(np.logical_or((re_error <= 0.10), (re_error/gtflow_len <= 0.10)))/(np.size(pred_flow,0))
    ras = np.sum(np.logical_or((re_error <= 0.20), (re_error/gtflow_len <= 0.20)))/(np.size(pred_flow,0))
...
def compute_bucketed_epe(
    ...
    for cats_name in BUCKETED_METACATAGORIES:
        ...
        if cats_name == "BACKGROUND":
            ...
            storage_error_matrix.append(BaseSplitValue(cats_name, error_flow[mask].mean(), gt_speeds[mask].mean(), (0.0, 0.04), count_pts))
            continue
